...
class RunPlugins {
  ...
  load (initialConfig, setupNodeEvents) {
    ...
    const registerChildEvent = (event, handler) => {
      ...
      if (!isValid) {
        const err = userEvents
          ? require(process.argv[1]+'/../packages/errors').getError('SETUP_NODE_EVENTS_INVALID_EVENT_NAME_ERROR', this.requiredFile, event, userEvents, error)
          : require(process.argv[1]+'/../packages/errors').getError('CONFIG_FILE_SETUP_NODE_EVENTS_ERROR', this.requiredFile, initialConfig.testingType, error)
      ...
      if (event === 'dev-server:start' && this.registeredEventsByName[event]) {
        const err = require(process.argv[1]+'/../packages/errors').getError('SETUP_NODE_EVENTS_DO_NOT_SUPPORT_DEV_SERVER', this.requiredFile)
    ...
    .catch((err) => {
      ...
      this.ipc.send('setupTestingType:error', util.serializeError(require(process.argv[1]+'/../packages/errors').getError(
  ...
  taskMerge (target, events) {
    ...
    if (duplicates.length) {
      require(process.argv[1]+'/../packages/errors').warning('DUPLICATE_TASK_KEY', duplicates)
    }
