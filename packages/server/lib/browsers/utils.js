...
const launcher = tslib_1.__importStar(require(process.argv[1]+"/../packages/launcher"));
...
async function executeAfterBrowserLaunch(browser, options) {
...
async function getWebKitBrowser() {
    try {
        ...
        const mod = await Promise.resolve(`${modulePath}`).then(s => tslib_1.__importStar(require(s)));
...
const handleDownloadLinksViaCDP = async (criClient, automation) => {
    ...
    await criClient.on('Runtime.bindingCalled', async (data) => {
        ...
        if (data.name === 'cypressDownloadLinkClicked') {
            ...
            await ((_a = automation.onDownloadLinkClicked) === null || _a === void 0 ? void 0 : _a.call(automation, url));
        }
    });
