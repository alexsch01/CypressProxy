...
class Automation {
    constructor(cyNamespace, cookieNamespace, screenshotsFolder, onBrowserPreRequest, onRequestEvent, onRequestServedFromCache, onRequestFailed, onDownloadLinkClicked) {
        this.onBrowserPreRequest = onBrowserPreRequest;
        this.onRequestEvent = onRequestEvent;
        this.onRequestServedFromCache = onRequestServedFromCache;
        this.onRequestFailed = onRequestFailed;
        this.onDownloadLinkClicked = onDownloadLinkClicked;
        ...
        this.screenshot = (0, screenshot_1.Screenshot)(screenshotsFolder);
    }
    reset() {
        this.middleware = this.initializeMiddleware();
    }
    automationValve(message, fn) {
        return (msg, data) => {
            ...
            if (onReq) {
                return bluebird_1.default.resolve(onReq(msg, data));
            }
            // do the default
            return bluebird_1.default.resolve(this.requestAutomationResponse(msg, data, fn));
        };
    }
