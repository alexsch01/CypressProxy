...
const schema_1 = require(process.argv[1]+"/../packages/graphql/src/schema");
...
const graphql_1 = require(process.argv[1]+"/../packages/graphql");
...
function makeDataContext(options) {
    const ctx = new data_context_1.DataContext({
        ...
        projectApi: {
            ...
            async runSpec(spec) {
                open_project_1.openProject.changeUrlToSpec(spec);
            },
            routeToDebug(runNumber) {
                open_project_1.openProject.changeUrlToDebug(runNumber);
            },
        },
        electronApi: {
            openExternal(url) {
            ...
            createNotification(title, body) {
                return new electron_1.default.Notification({ title, body });
            },
        },
        localSettingsApi: {
