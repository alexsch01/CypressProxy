...
const mobx_runner_store_1 = require(process.argv[1]+"/../app/src/store/mobx-runner-store");
let runner;
let runnables;
const { _ } = Cypress;
function visitAndRenderReporter(studioEnabled = false, studioActive = false) {
    cy.fixture('runnables_memory').then((_runnables) => {
        runnables = _runnables;
    });
    runner = new events_1.EventEmitter();
    const runnerStore = new mobx_runner_store_1.MobxRunnerStore('e2e');
    runnerStore.setSpec({
        name: 'foo.js',
