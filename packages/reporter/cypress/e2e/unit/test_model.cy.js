...
const test_model_1 = tslib_1.__importDefault(require("../../../src/test/test-model"));
const { _ } = Cypress;
const createTest = (props = {}, store = {}) => {
    const defaults = {
        currentRetry: 0,
        id: 'r3',
        prevAttempts: [],
    ...
    return new test_model_1.default(_.defaults(props, defaults), 0, store);
};
...
describe('Test model', () => {
    ...
    context('.isLongRunning', () => {
        ...
        it('becomes long running if active and has a long running command', () => {
            const test = createTest();
            test.start({});
            const command = test.addLog(createCommand());
    ...
    context('#addLog', () => {
        ...
        it('adds the command to an existing hook if it already exists', () => {
            ...
            const command = test.addLog(commandProps);
    ...
    context('#finish', () => {
        ...
        it('updates the state of the test', () => {
            const test = createTest();
            test.finish({ state: 'failed' }, false);
            expect(test.state).to.equal('failed');
        });
        it('updates the test err', () => {
            const test = createTest();
            test.finish({ err: { name: 'SomeError' } }, false);
        ...
        it('sets the hook to failed if it exists', () => {
            ...
            test.finish({ state: 'failed', failedFromHookId: 'h1', err: { message: 'foo' } }, false);
            expect(test.lastAttempt.hooks[1].failed).to.be.true;
        });
        it('does not throw error if hook does not exist', () => {
            const test = createTest();
            expect(() => {
                test.finish({ hookId: 'h1' }, false);
            }).not.to.throw();
        });
    });
    context('#commandMatchingErr', () => {
        it('returns last command matching the error', () => {
            const test = createTest({ err: { message: 'SomeError' }, hooks: [
                    { hookId: 'h1', hookName: 'before each' },
                    { hookId: 'h2', hookName: 'before each' },
                ] });
            test.addLog(createCommand({ err: { message: 'SomeError' }, hookId: 'h1' }));
            test.addLog(createCommand({ err: {}, hookId: 'h1' }));
            test.addLog(createCommand({ err: { message: 'SomeError' }, hookId: 'h1' }));
            test.addLog(createCommand({ err: {}, hookId: 'h2' }));
            test.addLog(createCommand({ name: 'The One', err: { message: 'SomeError' }, hookId: 'h2' }));
